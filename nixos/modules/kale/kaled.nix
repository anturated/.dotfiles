  ...
  kaleDaemon = pkgs.writeTextFile {
    ...
    text = ''
      ...
      class kaleDaemon(dbus.service.Object):
          def __init__(self):
              bus_name = dbus.service.BusName('com.anturated.kale', bus=dbus.SystemBus())
              dbus.service.Object.__init__(self, bus_name, '/com/anturated/kale')
              self.clients = set()
          ...
          @dbus.service.method('com.anturated.kale', in_signature='i')
...
in
{
  ...
  services.dbus.packages = [
    (pkgs.writeTextFile {
      name = "kale-dbus-conf";
      destination = "/share/dbus-1/system.d/com.anturated.kale.conf";
      text = ''
        <!DOCTYPE busconfig PUBLIC "-//freedesktop//DTD D-BUS Bus Configuration 1.0//EN"
         "http://www.freedesktop.org/standards/dbus/1.0/busconfig.dtd">
        <busconfig>
          <policy user="root">
            <allow own="com.anturated.kale"/>
          </policy>
          <policy context="default">
            <allow send_destination="com.anturated.kale"/>
          </policy>
        </busconfig>
      '';
    })
  ...
  systemd.services.kale = {
    description = "Daemon for TLP bs to use with antg";
    after = [ "dbus.service" ];
    wantedBy = [ "multi-user.target" ];
    serviceConfig = {
      ExecStart = "${kaleDaemon}/bin/antd.py";
      Restart = "always";
    };
  };
}
